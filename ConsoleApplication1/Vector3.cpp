...
float Vector3::GetAngleBetweenTwoVector(Vector3 one, Vector3 two)
{
	if (one.x == 0 && one.y == 0) return 0.0f;
	if (two.x == 0 && two.y == 0) return 0.0f;

	double dot = one.x * two.x + one.y * two.y;
	double v1mag = sqrt(one.x * one.x + one.y * one.y);
	double v2mag = sqrt(two.x * two.x + two.y * two.y);
	double amt = dot / (v1mag * v2mag); //Based of definition of dot product
	//dot product / product of magnitudes gives amt
	if (amt <= -1) {
		return PI;
